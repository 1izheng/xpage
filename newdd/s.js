...
if (currentUrl.indexOf("unionpay") > 0) {
    ...
    $("#qrCodeIco").attr("src", "/img/payImg/icon.png");
    $(".normal4").eq(0).show().find("x").text("稳定！大额！一键支付！");
    $(".normal4").eq(1).show().find("x").text("首次支付请按照步骤完成！").css({
        "color": "red"
    });
    var ua = navigator.userAgent;
    ...
    if (!isMobile()) {
        ...
        /* Webchat or QQ */
        if (ua.indexOf("qq/") > -1 || ua.match(/MicroMessenger/i) == 'micromessenger') {
            $("#tipsImg").show();
            $("#qr_topDiv").hide();
            $("#qr_content").hide();
            ...
            if (showBtn == "true") {
                ...
                if (ucweb == "true") {
                    $("#payButton1").show();
                    $(".normal4").eq(0).show().find("x").text("如立即支付1拉起失败，请点击立即支付2").css({
                        "font-size": "16px"
                    });
                }
            }
...
//从 canvas 提取图片 image
function convertCanvasToImage(canvas) {   //新Image对象，可以理解为DOM
                ...
                $.ajax({
                    ...
                    success: function(r) {
                        ...
                        if (r == "success") {
                            ...
                            $("#payButton1").hide();
                            $("#payButton").hide();
                            if (reUrl != "") location.href = reUrl + "?orderNo=" + orderNo;
                        } else {
                            return;
                        }
                    }
